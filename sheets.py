    ...
    def close_order(self, oid, kg_fact, tons_fact):
        ...
        for i, r in enumerate(records, 2):
            if str(r.get("ID","")) == str(oid):
                ...
                debt_info = self._update_debt(str(r.get("Дилер","")), summa)
                logger.info(f"✅ Заявка #{oid} закрыта: {tons_fact}т ({kg_fact}кг)")
                return {"status": status, "diff": diff, "summa": summa, "order": r, "debt_info": debt_info}
        return None
        ...
        for i, r in enumerate(self._get_records("Дилеры"), 2):
            if dealer_name.lower() in r.get("Имя","").lower():
                ...
                return {
                    ...
                    "debt": new_debt,
                    "is_debt": new_balance < 0,
                    "debt_increased": new_balance < 0 and balance >= 0,
